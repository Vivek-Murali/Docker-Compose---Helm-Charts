...
function buildImages() {
  ...
  then
    ...
    docker build \
      ...
      -f docker/extendeds/spark-base/Dockerfile \
      -t spark-base:${SPARK_VERSION} .

    docker build \
      --build-arg build_date="${BUILD_DATE}" \
      --build-arg spark_version="${SPARK_VERSION}" \
      -f docker/extended/spark-master/Dockerfile \
      -t spark-master:${SPARK_VERSION} .

    docker build \
      --build-arg build_date="${BUILD_DATE}" \
      --build-arg spark_version="${SPARK_VERSION}" \
      -f docker/extended/spark-worker/Dockerfile \
  ...
  then
    docker build \
      ...
      -f docker/extended/jupyterlab/Dockerfile \
